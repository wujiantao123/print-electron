...
const USER_NAME = "364222180@qq.com";
const USER_KEY = "7b92cb24871145dfa1827a7f578a0f7f";
const OK_PRINTER_SN = "02GGSSTUVAA9848";
/**
 * 小票打印字体对齐样例，不支持金额播报
...
************商品信息************
${goodsList.map((item) =>
  formatter.formatPrintOrderItem(item.goodsName, item.number, item.amount)
)}
--------------------------------
配送费:￥${amountLogistics}
--------------------------------
...
async function printComplexReceipt() {
  ...
  printContent =
    printContent + formatter.formatPrintOrderItem("可乐鸡翅", 2, 9.99);
  printContent =
    printContent + formatter.formatPrintOrderItem("水煮鱼特辣", 1, 108.0);
  printContent =
    printContent +
    formatter.formatPrintOrderItem("豪华版超级无敌龙虾炒饭", 1, 99.9);
  printContent =
    printContent + formatter.formatPrintOrderItem("炭烤鳕鱼", 5, 19.99);
  printContent = printContent + util.strRepeat("-", 32) + "<BR>";
  ...
  printContent =
    printContent + "<C>" + "<QR>https://www.xpyun.net</QR>" + "</C>";
...
module.exports = {
  printFontAlign,
  printFontAlignVoiceSupport,
  printComplexReceipt,
  printComplexReceiptVoiceSupport,
  printLabel,
};
